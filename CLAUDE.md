...
| `stdlib/src/adam_tools/prepare_names.py` | Names dataset download and encoding |
| `tests/test_e2e.py` | Pytest E2E tests with inline `// expect:` assertions |
| `tests/test_regression.py` | Regression tests against baseline snapshots |
...
- **Transformer demo**: `examples/transformer.adam` trains a single-layer, single-head transformer on 32K names and generates new names. Uses 3D tensors, batched matmul, causal masking, and manual backprop through attention. See [docs/transformer.md](docs/transformer.md).
